...
export default function ListModal({ item, onClose, onSave, onDelete }) {
  ...
  const changeDescription = React.useCallback((evt) => {
    copyItem.description = evt.target.value;
    setCopyItems({ ...copyItem });
  });
  ...
  const changeCalories = React.useCallback((evt) => {
    copyItem.calories = evt.target.value;
    setCopyItems({ ...copyItem });
  });

  return (
    <Dialog open onClose={onClose} onBackdropClick={onClose}>
      <DialogTitle className="title">Log Meal</DialogTitle>
      <DialogContent sx={{ width: 400, paddingTop: '1em !important' }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <TextField fullWidth onChange={(evt) => changeDescription(evt)} className="title" id="standard-basic" value={copyItem.description}
              multiline
              maxRows={4} label="Description" />
          </Grid>
          <Grid item xs={6}>
            <TextField fullWidth onChange={(evt) => changeCalories(evt)} className="title" id="standard-basic" value={copyItem.calories} label="Calories" />
          </Grid>
          <Grid item xs={6}>
